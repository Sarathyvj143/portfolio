...
export default class ArticleDataWrapper {
    ...
    constructor(section, rawData, language, theme, id) {
        ...
        this._rawData = rawData // Store the raw data for custom components
    ...
    _parseItems(rawData, language, theme) {
        ...
        // Special handling for blog component items
        if (component === "Blog") {
            console.log("Processing Blog component items:", rawItems);
        }
        
        return rawItems.map((rawItem, key) => {
            // Create a wrapper with the raw item data
            const wrapper = new ArticleItemDataWrapper(this, rawItem, language, theme, key + 1)
            
            // For Blog components, ensure blogData is properly set
            if (component === "Blog" && rawItem.blogData) {
                wrapper.blogData = rawItem.blogData
                console.log("Set blogData on wrapper:", wrapper.blogData);
            }
